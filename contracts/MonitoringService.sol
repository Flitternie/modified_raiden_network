...
contract MonitoringService is Utils {
	...
	event Penalty(address[] indexed ms_addresses, uint amount)
    ...
    internal
    {
        ...
        address raiden_node_address = recoverAddressFromRewardOrPenaltyProof(
            channel_identifier,
            reward_amount,
	...
	function penalty(
		address token_network_address,
		address[] ms_addresses,					// The MS addresses that need to be penalized
		bytes[] penalty_proof_signature,		// For verification
		
		address closing_participant,
		address non_closing_participant,		// For channel identification
		uint256 penalty_amount,

		bytes32 balance_hash,					// BS info.
		uint256 nonce,
		bytes32 additional_hash,
		bytes closing_signature,
		bytes non_closing_signature
	...
	{	
		int256 curNonce;
		bytes32 curBalanceHash;
		uint256 settle_block_number;
		
		// Identify the channel
		TokenNetwork token_network = TokenNetwork(token_network_address);
        bytes32 channel_identifier = token_network.getChannelIdentifier(closing_participant, non_closing_participant);
		( , settle_block_number, ) = token_network.getChannelInfo(closing_participant, non_closing_participant);
		
		// Verify the Balance Proof (necessary?)
		address recovered_non_closing_participant = token_network.recoverAddressFromBalanceProof(		
            channel_identifier,
            balance_hash,
            nonce,
            additional_hash,
		...
		address recovered_closing_participant = token_network.recoverAddressFromBalanceProof(
            channel_identifier,
            balance_hash,
            nonce,
            additional_hash,
		...
		// Penalty conditions check
		require (settle_block_number < block.number);
		(, , , curBalanceHash,curNonce) = token_network.getChannelParticipantInfo(closing_participant, non_closing_participant);
		require(curBalanceHash != balance_hash || curNonce != nonce);
		
		// Penalization
		require (ms_addresses.length == penalty_proof_signature.length);
		for (uint i = 0; i < ms_addresses.length; i++){
			address raiden_node_address = recoverAddressFromRewardOrPenaltyProof(
            channel_identifier,
            penalty_amount,
            token_network_address,
            token_network.chain_id(),
            nonce,
            penalty_proof_signature[i]
        );
        require(raiden_node_address == ms_addresses[i]);
		require(rsb.deposits(ms_addresses[i]) - penalty_amount < rsb.deposits(ms_addresses[i]));
		rsb.deposits(ms_addresses[i]) -= penalty_amount;
		require(balances[reward.non_closing_participant] + penalty_amount > balances[reward.non_closing_participant]);
		balances[reward.non_closing_participant] += penalty_amount;
		/*
		What if the deposit is less than penalty amount?
			if (penalty_amount>balances[reward.ms_addresses[i]]){
				balances[reward.ms_addresses[i]] = 0;
				rsb.deposits( ms_addresses[i]) -= penalty_amount-balances[reward.ms_addresses[i]];
			}
		*/
		require(token_network.transferFrom( _rsb_address, address(this), penalty_amount));
		}
		emit Penalty(ms_addresses[], penalty_amount);
	}
	
	function recoverAddressFromRewardOrPenaltyProof(
