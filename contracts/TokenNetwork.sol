...
contract TokenNetwork is Utils {
	...
	*/
	...
	/// Deposit transfer from participant's one channel to another
	function depositTransfer(
		...
		address partnerTo,
        bytes participant_signature,
        bytes partnerFrom_signature
	)
		external
	{
		require(amount > 0);
				
		// Withdraw from the channel between participant and partnerFrom
		uint256 total_withdraw_From = getTotalWithdraw(participant, partnerFrom, amount);
		this.setTotalWithdraw(participant, total_withdraw_From, partnerFrom, participant_signature, partnerFrom_signature);
		
		// Deposit to the channel between participant and partnerTo
		uint256 total_deposit_To = getTotalDeposit(participant, partnerTo, amount);
		setTotalDeposit(participant, total_deposit_To, partnerTo);	
	}
	...
	{
		bytes32 channel_identifier_To;
	
		channel_identifier_To = getChannelIdentifier(participant, partnerTo);
		Channel storage channel_To = channels[channel_identifier_To];
		Participant storage participantTo_state = channel_To.participants[participant];
		require(channel_To.state == 1);	
		return participantTo_state.deposit + amount;
	}
	...
	function getTotalWithdraw(
		...
		view 
		internal 
		returns (uint256 TotalWithdraw)
	{
		bytes32 channel_identifier_From;

		channel_identifier_From = getChannelIdentifier(participant, partnerFrom);
		Channel storage channel_From = channels[channel_identifier_From];
		Participant storage participantFrom_state = channel_From.participants[participant];
		require(channel_From.state == 1);	
		return participantFrom_state.withdrawn_amount + amount;
	}
	...
	function verifyBalanceProof(
        ...
        public
	{
		bytes32 channel_identifier = getChannelIdentifier(closing_participant, non_closing_participant);
		
		address recovered_non_closing_participant = recoverAddressFromBalanceProof(		
            channel_identifier,
            balance_hash,
            nonce,
            additional_hash,
		...
		address recovered_closing_participant = recoverAddressFromBalanceProof(
            channel_identifier,
            balance_hash,
            nonce,
            additional_hash,
		...
		external
	{
		...
		bytes32 channel_identifier_From;
		bytes32 channel_identifier_To;
		uint256 total_withdraw_From;
		uint256 total_deposit_To;
		...
		total_withdraw_From = participantFrom_state.withdrawn_amount + amount;
		setTotalWithdraw(
			participant,
			total_withdraw_From,
			partnerFrom,
			participant_signature,
			partnerFrom_signature
		);
		...
		total_deposit_To = participantTo_state.deposit + amount;
		setTotalDeposit(participant, total_deposit_To, partnerTo);
